...
flit::Variant LaghosTest<double>::run_impl(const std::vector<double> &ti) {
   ...
   // **************************************************************************
   // Mult RAP MPI test
   if (mult) throw std::runtime_error("error");
      ...
      if (last_step || (ti % vis_steps) == 0)
      {
         ...
         if (gfprint)
         {
            ...
            mesh_name << basename << "_" << ti
                      << "_mesh." << setfill('0') << setw(6) << 0;
            rho_name  << basename << "_" << ti
                      << "_rho." << setfill('0') << setw(6) << 0;
            v_name << basename << "_" << ti
                   << "_v." << setfill('0') << setw(6) << 0;
            e_name << basename << "_" << ti
                   << "_e." << setfill('0') << setw(6) << 0;
   ...
   oper.PrintTimingData(true, steps);
